...
export default function IndividualPage() {
  ...
  // If exam number is specified, load the exam component
  if (examNumber) {
    const examNum = parseInt(examNumber)
    if (availableExams.length > 0 && availableExams.includes(examNum)) {
      // Import and render the exam component dynamically
      return <IndividualExamComponent examNumber={examNum} mode={mode} />
...
// Component for individual exam (full implementation)
function IndividualExamComponent({ examNumber, mode }: { examNumber: number, mode: string }) {
  const router = useRouter()
  ...
  const finishExam = () => {
    ...
    const originalMode = isPracticeMode ? 'practice' : 'test'
    router.push(`/results?id=${resultsId}&mode=individual&examNumber=${examNumber}&originalMode=${originalMode}`)
  }
